...
public class DialogManager : MonoBehaviour
{
	...
	public int ShowSelectDialog (string title, string msg, Action<bool> del)
	{
		int id;
		#if UNITY_EDITOR
		Debug.LogWarning("Editor用のダイアログはありません");
		id = 0;
		#elif UNITY_ANDROID
		using (AndroidJavaClass cls = new AndroidJavaClass("unity.plugins.dialog.DialogManager")) {
	...
	public void DissmissDialog(int id) {
		Debug.LogWarning ("DialogManage.DissmissDialog:" + id);
		#if UNITY_EDITOR
	...
	{
		Debug.LogWarning ("DialogManage.OnSubmit:" + idStr);
	...
	{
		Debug.LogWarning ("DialogManage.OnCancel:" + idStr);
		int id = int.Parse (idStr);
		if (_delegates.ContainsKey (id)) {
			_delegates [id] (false);
			_delegates.Remove (id);
